...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        self.encoder_embedding_dim = hparams.encoder_embedding_dim + hparams.token_embedding_size + hparams.emotion_embedding_dim
...
class Tacotron2(nn.Module):
    ...
    def parse_batch(self, batch):
        text_padded, input_lengths, mel_padded, gate_padded, \
            output_lengths, emotion_ids = batch
    ...
    def forward(self, inputs):
        inputs, input_lengths, targets, max_len, \
            output_lengths, emotion_ids = inputs
        ...
        embedded_emotions = embedded_emotions.repeat(1, embedded_text.size(1), 1)

        encoder_outputs = torch.cat(
            (embedded_text, embedded_gst, embedded_emotions), dim=2)
    ...
    def inference(self, inputs):
        text, style_input, emotion_ids = inputs
        ...
        if hasattr(self, 'gst'):
            ...
            encoder_outputs = torch.cat(
                (embedded_text, embedded_emotions), dim=2)
    ...
    def inference_noattention(self, inputs):
        text, style_input, emotion_ids, attention_map = inputs # Rhythm IS the attention map
        embedded_inputs = self.embedding(text).transpose(1, 2)
        embedded_text = self.encoder.inference(embedded_inputs)
        embedded_emotions = self.emotion_embedding(emotion_ids)[:, None]
        if hasattr(self, 'gst'):
        ...
        mel_outputs, gate_outputs, alignments = self.decoder.inference_noattention(
            encoder_outputs, attention_map) # attention map
